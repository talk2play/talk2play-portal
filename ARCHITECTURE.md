...
2. **Solo `scripts/update.py` habla con la red en desarrollo.** El navegador carga
   exclusivamente ficheros locales más las miniaturas de `i.ytimg.com`; ningún
   `fetch()` en `app.js`.
...
5. **`data/videos.js` es generado: no se edita a mano.** Lo que esté mal ahí se
   arregla en `update.py` y se regenera.
